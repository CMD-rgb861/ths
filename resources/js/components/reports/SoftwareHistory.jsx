...
export default function SoftwareHistory() {
  ...
  const filtered = jobs.filter((job) => {
    ...
    // Only show jobs with "Software" in categories
    const cats = job.categories?.map((c) => c.name) || [];
    if (!cats.includes('Software')) return false;
  ...
  const exportCsv = () => {
    window.location.href = '/job-orders/export/csv?type=software-history';
  };
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
          <div>
            ...
            <p className="mt-1 text-gray-600">
              View the history of software names from past job orders. Search by software name.
            </p>
          </div>
